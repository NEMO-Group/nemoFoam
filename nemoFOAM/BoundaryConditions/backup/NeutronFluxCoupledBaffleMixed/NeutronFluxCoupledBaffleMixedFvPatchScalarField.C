    ...
    contactRes_(0.0)
{
    ...
    if (dict.found("thicknessLayers"))
    {
        ...
        if (thicknessLayers_.size() > 0)
        {
            ...
            {
                contactRes_ += thicknessLayers_[iLayer]/DLayers_[iLayer];
            }
...
:
    ...
    DLayers_(ptf.DLayers_),
    //DFieldName_(ptf.DFieldName_),
...
:
    ...
    DLayers_(wtcsf.DLayers_),
   // DFieldName_(wtcsf.DFieldName_),
...
void NeutronFluxCoupledBaffleMixedFvPatchScalarField::updateCoeffs()
{
    ...
    if (contactRes_ == 0.0)
    {
        ...
        nbrKDelta.ref() = nbrField.D(nbrField)*nbrPatch.deltaCoeffs();
    }
    else
    {
    ...
    tmp<scalarField> myKDelta = D(*this)*patch().deltaCoeffs();
...
) const
{
    ...
    writeEntry(os, "DLayers", DLayers_);
