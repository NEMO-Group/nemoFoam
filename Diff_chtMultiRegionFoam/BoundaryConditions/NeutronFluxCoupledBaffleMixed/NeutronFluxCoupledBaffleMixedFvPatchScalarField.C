...
/*
    const fvPatchScalarField& nbrD =
        nbrPatch.lookupfvPatchField<volScalarField, scalar>(nbrDFieldName_);
*/

const volScalarField& DDD = nbrMesh.lookupObject<volScalarField>(nbrDFieldName_);
const fvPatchScalarField& nbrD = DDD.boundaryField()[samplePatchi];
...
//        nbrKDelta.ref() = nbrField.nbrD(nbrField)*nbrPatch.deltaCoeffs();
        nbrKDelta.ref() = nbrD*nbrPatch.deltaCoeffs();


    mpp.distribute(nbrIntFld.ref());
    mpp.distribute(nbrKDelta.ref());

    tmp<scalarField> myKDelta = myD(*this)*patch().deltaCoeffs();
